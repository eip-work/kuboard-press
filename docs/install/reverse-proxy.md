---
description: 通过 Nginx 反向代理 / Ingress 为 Kuboard 配置 HTTPS 与 WebSocket
---

# 反向代理

本文介绍如何通过 Nginx 或 Kubernetes Ingress 为 Kuboard 配置 HTTPS 与 WebSocket（终端 / 日志功能依赖 WebSocket）。
...
如果使用 [nginx-ingress](https://docs.nginx.com/nginx-ingress-controller/overview/) 作为您的 Ingress Controller，其他类型的 Ingress 控制器（如 traefik，ingress-nginx 等）请自行参考其官方文档，了解如何启用 WebSocket 并配置 PassHeader
