...
* 设置内置 ETCD 的启动参数 `--quota-backend-bytes=8589934592`
* 如果集群启用了 PureLB，则在 Service 编辑页的服务类型字段启用 `LoadBalancer` 选项
...
* 修订部分情况下（集群 `/health` 返回大写 OK 时），导致集群状态判断错误的问题。
* 修订部分情况下，CR 菜单权限判定错误的问题。
